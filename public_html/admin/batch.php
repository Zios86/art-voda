...
try {
    ...
    if($action==='coordinates'){
        ...
        foreach($ids as $id){$point=$decoded[(string)$id]??null;$lat=filter_var($point[0]??null,FILTER_VALIDATE_FLOAT);$lng=filter_var($point[1]??null,FILTER_VALIDATE_FLOAT);if($lat===false||$lng===false||$lat<59||$lat>61||$lng<29||$lng>32)continue;$statement->execute([$lat,$lng,$id]);}
    }elseif($action==='area'){
    ...
    foreach($ids as $id){app_kiosk_store_version($pdo,$id,'batch-'.$action);$audit->execute([$id,'batch-'.$action,(string)($config['admin']['username']??'admin'),hash_hmac('sha256',(string)($_SERVER['REMOTE_ADDR']??''),$auditKey)]);}
    $pdo->commit();app_admin_alert('Пакетное изменение автоматов','Действие: '.$action.', точек: '.count($ids));app_kiosk_clear_api_cache();header('Location: /admin/map.php?saved='.count($ids));
