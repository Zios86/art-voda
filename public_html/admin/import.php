...
/** CSV: подробное сравнение, отдельное подтверждение и полный аудит реально изменённых строк. */
...
if ($action === 'confirm') {
    ...
    $rows = array_values(array_filter(is_array($preview['rows'] ?? null) ? $preview['rows'] : [], static fn (array $row): bool => ($row['result'] ?? '') !== 'same'));
    if (!$rows) { http_response_code(422); exit('В файле нет изменений для применения.'); }
    try {
        $pdo = app_pdo();
        ...
        foreach ($rows as $row) {
            ...
            $rowAction = ($row['result'] ?? '') === 'add' ? 'import-create' : 'import-update';
            app_kiosk_store_version($pdo, $id, $rowAction);
            app_kiosk_audit($pdo, $id, $rowAction);
        }
        app_kiosk_audit($pdo, null, 'import-complete');
        $pdo->commit();
        app_kiosk_sync_public_json($pdo);
        if (count($rows) >= 20) app_admin_alert('Массовый импорт CSV', 'Изменено строк: ' . count($rows));
        unset($_SESSION['csv_preview']);
        header('Location: /admin/?imported=' . count($rows));
        exit;
...
if (array_slice($header, 0, 6) !== ['Номер','Адрес','Город/район','Широта','Долгота','Режим работы']) {
    http_response_code(422); exit('Используйте CSV, скачанный из этой админки.');
}
...
while (($values = fgetcsv($handle, 0, ';', '"', '')) !== false) {
    ...
    try {
        $data = app_kiosk_form_data([
            'machine_number'=>$values[0], 'address'=>$values[1], 'area'=>$values[2], 'latitude'=>$values[3],
            'longitude'=>$values[4], 'schedule'=>$values[5], 'metro'=>$values[6] ?? '', 'landmark'=>$values[7] ?? '',
        ]);
    ...
    } catch (InvalidArgumentException $error) { $errors[] = ['line'=>$line,'message'=>$error->getMessage()]; }
...
$labels = ['address'=>'Адрес','area'=>'Город/район','latitude'=>'Широта','longitude'=>'Долгота','schedule'=>'Режим работы','metro'=>'Метро','landmark'=>'Ориентир','photo_url'=>'Фотография'];
...
foreach ($rows as &$row) {
    ...
    if (!$old) $row['result'] = 'add';
        ...
        foreach (array_keys($labels) as $field) {
            $newValue = $field === 'photo_url' && $row[$field] === '' ? (string) $old[$field] : (string) $row[$field];
            if ((string) $old[$field] !== $newValue) $row['changes'][$field] = ['before'=>(string)$old[$field],'after'=>$newValue];
        }
        $row['result'] = $row['changes'] ? 'update' : 'same';
    }
    $stats[$row['result']]++;
}
unset($row);
...
?><!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Проверка CSV — Киосквода</title><link rel="stylesheet" href="/css/admin.css?v=20260901-1"></head><body>
<header class="admin-header"><div><strong>Киосквода</strong><span>Проверка CSV</span></div><nav><a href="/admin/">Отменить</a></nav></header>
<main class="admin-page"><div class="admin-page__heading"><div><p>Предпросмотр импорта</p><h1>Проверьте каждое изменение</h1></div><span><?=count($rows)?> корректных строк</span></div>
...
<div class="admin-table-wrap"><table><thead><tr><th>Строка</th><th>Результат</th><th>№</th><th>Адрес</th><th>Точные отличия</th></tr></thead><tbody><?php foreach($rows as $row):?><tr><td><?=(int)$row['line']?></td><td><span class="result-badge result-badge--<?=app_h($row['result'])?>"><?=app_h(['add'=>'Добавить','update'=>'Изменить','same'=>'Без изменений'][$row['result']])?></span></td><td><?=(int)$row['number']?></td><td><?=app_h($row['address'])?></td><td><?php if($row['result']==='add'):?>Новая точка<?php elseif(!$row['changes']):?>Нет<?php else:?><ul class="diff-list"><?php foreach($row['changes'] as $field=>$change):?><li><strong><?=app_h($labels[$field])?>:</strong> <?=app_h($change['before']?:'—')?> → <?=app_h($change['after']?:'—')?></li><?php endforeach;?></ul><?php endif;?></td></tr><?php endforeach;?></tbody></table></div>
<?php $applyCount=$stats['add']+$stats['update'];?><form class="confirm-bar" method="post"><input type="hidden" name="csrf" value="<?=app_h(app_csrf())?>"><input type="hidden" name="action" value="confirm"><input type="hidden" name="preview_token" value="<?=app_h($token)?>"><label>Повторите пароль<input type="password" name="admin_password" required autocomplete="current-password"></label><a href="/admin/">Отменить</a><button type="submit"<?=$applyCount?'':' disabled'?>>Применить <?=$applyCount?> изменений</button></form></main></body></html>
