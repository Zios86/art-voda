...
if (!is_array($batchResult) || (int) ($batchResult['created_at'] ?? 0) < time() - 600) $batchResult = null;
...
<section class="bulk-map-sidebar"><div><p>Массовое управление</p><h1>Все автоматы</h1><small>Перетащите метку или выберите точки для общего изменения.</small></div><?php if($batchResult):?><details class="batch-result" open><summary>Изменено точек: <?=count($batchResult['ids']??[])?></summary><ul><?php foreach(($batchResult['labels']??[]) as $label):?><li><?=app_h((string)$label)?></li><?php endforeach;?></ul></details><?php endif;?><label>Поиск<input id="bulk-search" type="search" placeholder="Номер или адрес"></label><div id="bulk-list" class="bulk-kiosk-list"></div></section>
