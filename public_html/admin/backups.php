...
<header class="admin-header"><div><strong>Киосквода</strong><span>Резервные копии</span></div><nav><a href="/admin/">Панель владельца</a><a href="/admin/backup.php">Скачать полную копию с фото</a></nav></header>
<main class="admin-page"><div class="admin-page__heading"><div><p>Закрытое хранилище</p><h1>Резервные копии</h1></div><span><?=count($files)?> копий</span></div><p class="admin-notice">Автоматические копии содержат данные базы. Кнопка «Скачать полную копию с фото» создаёт ZIP с базой, используемыми фотографиями и SHA-256. Для сравнения повторите пароль.</p><?php if(isset($_GET['restored'])):?><p class="admin-success">Восстановлено точек: <?=max(0,(int)$_GET['restored'])?>.</p><?php endif;?>
